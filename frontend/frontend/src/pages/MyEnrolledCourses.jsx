...
function MyEnrolledCourses() {
   ...
   useEffect(() => { const fetchCourses = async () => { 
       try { let res; if (userData?.role === "student") 
       { res = await axios.get( ${serverUrl}/api/course/getEnrolledCourses, 
   { withCredentials: true } );
       } 
   else { res = await axios.get( ${serverUrl}/api/course/getcreator, 
   { withCredentials: true });
        } setCourses(res.data);
           } 
   catch (error) { 
       console.log("Error fetching courses:", error.response?.data || error.message); } }; if (userData)
   { fetchCourses();
   }
                   }, [userData]);
